...
const generateNavigationProgram = (schema: AppSchema): t.Program => {
  ...
  const tabScreens = Object.entries(navigation.tabs).map(([tab, pages]) =>
    ...
    config: {
      screens: {
        ...(tabScreens.length > 0
          ? {
              Tabs: {
                screens: Object.fromEntries(
                  Object.values(navigation.tabs).map((tab): [string, any] =>
          ...
          : {}),
        ...Object.fromEntries(
          navigation.stack.map((page) => [page.name, page.navigation.path])
        ),
      },
    },
  });
            ...
            t.jsxOpeningElement(
              ...
              [
                t.jsxAttribute(
                  ...
                  t.jsxExpressionContainer(linkingProp)
                ),
              ],
              false
            ),
